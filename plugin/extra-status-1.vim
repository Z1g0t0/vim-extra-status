...
"
" Everything else is literal text, so wrappers like {} [] are just written
" around the token in the format string.
"
...
function! StatusRegister(reg)
    ...
    let l:trunc = s:Trunc()
    " Clamp to the real window width so the register content never grows past the statusline and overlaps the other components on splits/resizes
